...
namespace Elin_Mod
{
	public class TraitModCombine : TraitCrafter
	{
		public override string IdSource => "Ex_ModCombine";
			...
			switch ( idx) {
				...
				default:
					if ( ret ) {
						ret = false;
						// 0番と同一の種別か調べる.
						var mod1 = LayerDragGrid.Instance.buttons[0].Card?.trait as TraitMod;
						var mod2 = c.trait as TraitMod;
		...
		public override Thing Craft(AI_UseCrafter ai) {
			...
			int combineCost = Mathf.FloorToInt(currentLv * config.CombineModCost.Value);

			textMng.SetUserData(0, combineCost);
			textMng.SetUserData(1, haveCost);
			var bodyText = textMng.GetText(eTextID.Dialog_CombineModQ);
